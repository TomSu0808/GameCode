...
class generate_grp(object):
    ...
    def generate_mesh_grp_window(self):
        ...
        cmds.window( "mesh_windowd", title = "Choose Nodel Type - Card", exists=True, widthHeight=(320, 50 ))
...
class plane_mesh_transform(object):
    ### Change Plane Width
...
class arc_mesh_transform(object):
    def get_nodes_in_history_by_type(typ, selection):
        nodes = []
        for obj in selection:
            for node in cmds.listHistory(obj):
